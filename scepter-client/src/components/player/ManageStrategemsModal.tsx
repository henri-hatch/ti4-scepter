...
type ManageStrategemsModalProps = {
  isOpen: boolean
  onClose: () => void
  owned: PlayerStrategem[]
  available: StrategemDefinition[]
  onAdd: (card: StrategemDefinition) => void
  onRemove: (card: PlayerStrategem) => void
  busyKey?: string | null
}

function ManageStrategemsModal({
  isOpen,
  onClose,
  owned,
  available,
  ...
  const filteredOwned = useMemo(() => {
    ...
    return owned.filter((card) => card.name.toLowerCase().includes(search))
  }, [owned, search])
  ...
  const filteredAvailable = useMemo(() => {
    ...
    return available.filter((card) => card.name.toLowerCase().includes(search))
  }, [available, search])
            ...
            <div className="manage-exploration-columns">
              <div className="manage-exploration-column">
                ...
                {filteredOwned.length === 0 ? (
                  <div className="manage-exploration-empty">No strategems assigned.</div>
                ) : (
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="manage-exploration-card-meta">
                            ...
                            <div className="manage-exploration-card-type">Trade Goods: {card.tradeGoods}</div>
              ...
              <div className="manage-exploration-column">
                ...
                {filteredAvailable.length === 0 ? (
                  <div className="manage-exploration-empty">No strategems available.</div>
                ) : (
                  <div className="manage-exploration-grid">
...
export default ManageStrategemsModal
